...
class ChatbotInsights {
  ...
  addInsightButtons() {
    ...
    const buttons = [
      {
        ...
        query: "show me today's waste collection status",
      },
      { text: "Bin Conditions", query: "show me overflowing bins" },
      {
        text: "Truck Routes",
        query: "show me route progress of collection trucks",
      },
    ...
    const firstBotMessage =
      this.messagesContainer.querySelector(".message.bot");
    if (firstBotMessage) {
  ...
  handleInsightQuery(query) {
    ...
    if (query.includes("collection status")) {
      ...
      response =
        "I'm not sure how to help with that. Please try one of the quick access buttons for waste collection insights.";
    }
  ...
  generateCollectionStatusResponse() {
    const collectedBins = this.wasteData.bins.filter(
      (bin) => bin.isCollected
    ).length;
    const totalBins = this.wasteData.bins.length;
    const uncollectedBins = totalBins - collectedBins;
    const criticalBins = this.wasteData.bins.filter(
      (bin) => bin.status === "Critical"
    ).length;

    return (
      `📊 **Today's Waste Collection Status**\n\n` +
      `- Bins collected: ${collectedBins}/${totalBins} (${Math.round(
        (collectedBins / totalBins) * 100
      )}%)\n` +
      `- Bins awaiting collection: ${uncollectedBins}\n` +
      `- Overflowing bins: ${criticalBins}\n` +
      `- Active collection trucks: ${
        this.wasteData.trucks.filter((truck) => truck.status === "On Route")
          .length
      }\n\n` +
  ...
  generateBinConditionsResponse() {
    ...
    const warningBins = this.wasteData.bins.filter(
    ...
    if (criticalBins.length > 0) {
      ...
      criticalBins.forEach((bin) => {
        response += `- ${bin.name}: ${bin.fillLevel}% full, ${
          bin.isCollected ? "collected" : "not collected"
        }\n`;
      });
      response += "\n";
    } else {
    ...
    this.wasteData.trucks.forEach((truck) => {
      const progressPercent =
        truck.completedBins > 0
          ? Math.round((truck.completedBins / truck.totalBins) * 100)
          : 0;
      response +=
        `**${truck.name}**\n` +
        `- Status: ${truck.status}\n` +
  ...
  generateSensorAnomaliesResponse() {
    const anomalySensors = this.wasteData.sensors.filter(
    ...
    if (anomalySensors.length > 0) {
      ...
      anomalySensors.forEach((sensor) => {
        ...
        response +=
          `**${sensor.name} (${bin ? bin.name : "Unknown bin"})**\n` +
